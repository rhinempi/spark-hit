...
public class HelpParam {
    ...
    public void printHelp(){
        ...
        final String executable = System.getProperty("executable", "java -jar FragRec.jar");
        err.println("Name:");
        err.println("\tFragment Recruitment");
        err.println();
        err.println("Options:");
        formatter.printOptions(new PrintWriter(err, true), 85, parameter, 2, 3); /* print formatted parameters */
        err.println();
        err.println("Usage:");
        err.println(executable + " [parameters] -fastq input_query.fq -reference input_reference.fa -outfile output_file.txt");
        err.println();
        err.println("SPARK_HOME/bin/Spark-submit [spark params] \\");
        err.println("FragFec.jar [parameters] -fastq input_query.fq -reference input_reference.fa -outfile output_file.txt");
        err.println();
    }
}
